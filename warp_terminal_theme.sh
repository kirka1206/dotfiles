#!/bin/bash
# Theme Configuration
readonly THEME_NAME="kir_code"
readonly THEME_CONTENT='background: "#2d2b55"
foreground: "#ffffff"
terminal_colors:
  normal:
    magenta: "#ff2c70"
...
printf "${GREEN_BOLD}вњ”${RESET} ${BOLD}Installing theme:${RESET}${DIM} ${THEME_NAME}${RESET}\n"
# Check if WARP_THEME_DIR exists
if [ ! -d "${WARP_THEME_DIR}" ]; then
	printf "${GREEN_BOLD}вњ”${RESET} ${BOLD}Creating Warp Theme Directory:${RESET}${DIM} ${WARP_THEME_DIR}${RESET}\n"
	mkdir -p "${WARP_THEME_DIR}"
fi
# Check if theme file already exists
if [ -f "${WARP_THEME_DIR}/${THEME_NAME}.yaml" ]; then
	printf "${RED_BOLD}вњ—${RESET} ${BOLD}Theme already exists:${RESET} ${DIM}${WARP_THEME_DIR}/${THEME_NAME}.yaml\n\n"
...
printf "${GREEN_BOLD}вњ”${RESET} ${GREEN}Successfully installed the theme!\n\n"
